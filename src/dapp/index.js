...
(async () => {
  ...
  let contract = new Contract("localhost", () => {
      ...
      function(error, result) {
        if (error) {
          ...
          let els = document.querySelectorAll(
            `.${btoa(
              result.returnValues.timestamp + result.returnValues.flight
            )}`
          );
          console.log(els[els.length - 1]);
          els[els.length - 1].querySelector(".results").innerText =
            result.returnValues.status === "10"
              ? "10 - On time"
              : `${result.returnValues.status} - ${
                  STATUS_CODES.find(
                    code => code.code == result.returnValues.status
                  ).label
                }`;
        }
      }
      ...
      function(error, result) {
        if (error) {
          ...
          alert(
            ...
            } regarding flight ${result.returnValues.flight}`
          );
        }
      }
    );
    ...
    DOM.elid("contract-status").addEventListener("click", () => {
    ...
    DOM.elid("submit-oracle").addEventListener("click", () => {
      ...
      let flight = JSON.parse(
        document.querySelector("#flights-selector").value
      );
      // Write transaction
      contract.fetchFlightStatus(flight, (error, result) => {
        display(
          ...
          btoa(result.timestamp + result.flight)
        );
      });
    });
